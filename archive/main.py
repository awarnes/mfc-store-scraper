...
LOCAL_DATA_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "outputs")
...
def collect_data(save_to_file=True):
    ...
    """
    raw_data, missing_products = all_product_data(
        all_product_ids(HUMMINGBIRD_ALL_PRODUCTS_URL)
    )
    ...
    if save_to_file:
        with open(
            os.path.join(LOCAL_DATA_PATH, HUMMINGBIRD_DATA_FILE), "w", encoding="utf8"
        ) as jsonfile:
...
def format_data(raw_data, fieldnames: Dict, from_file_path=None):
    ...
    if from_file_path:
        raw_data = []
        with open(
            os.path.join(LOCAL_DATA_PATH, from_file_path), encoding="utf8"
        ) as data_file:
    ...
    for product in product_data:
        products_to_write.append(
            {key: product.get(key, "") for key in fieldnames.values()}
        )

    with open(
        os.path.join(LOCAL_DATA_PATH, HUMMINGBIRD_CSV_FILE), "w", encoding="utf8"
    ) as csvfile:
