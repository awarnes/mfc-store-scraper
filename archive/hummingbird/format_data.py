...
def format_products(product_data):
    ...
    for product in product_data:
        ...
        if is_multi_pack(primary_variant):
            new_product["Option1 Value"] = MULTI_PACK_CONVERTER[
                primary_variant["option1"]
            ]
            multi_pack = True
        ...
        new_product["Variant Price"] = get_product_price(
            primary_variant["price"], multi_pack_amount
        )
        ...
        for variant in product["variants"][1:]:
            ...
            new_variant["Variant Inventory Policy"] = (
                SHOPIFY_VARIANT_INVENTORY_POLICY_CONTINUE
            )
            new_variant["Variant Fulfillment Service"] = (
                SHOPIFY_VARIANT_FULFILLMENT_SERVICE_MANUAL
            )

            new_variant["Variant Price"] = get_product_price(
                variant["price"], multi_pack_amount
            )
