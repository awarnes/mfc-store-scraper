...
class ShopifyProduct:
    """Definition of a Shopify product in the CSV"""
    ...
    def csv_format(self, fields=None):
        ...
        return [value for (key, value) in self.__dict__.items() if key in fields]


class ShopifyCsv:
    "Managing Shopify CSV creation"
    ...
    def write_products(self, products: List[ShopifyProduct]):
        """Writes list of products to a CSV"""
        with open(
            f"{str(datetime.now()).replace(' ', 'T')}_shopify_upload.csv",
            "w",
            newline="",
            encoding="utf-8",
        ) as csvfile:
