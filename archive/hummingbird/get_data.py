...
def all_product_ids(query_url, list_of_product_ids=None):
    ...
    list_of_product_ids += [
        re.sub("product-", "", product.attrs["id"]) for product in available_products
    ]
...
def all_product_data(ids_to_search):
    ...
    for page_number, page in enumerate(paginated_ids):
        ...
        if response.ok:
            ...
            print(
                f"returned data for {len(response_json)} of {len(page)} products queried"
            )
            ...
            if len(response_json) != len(page):
                missing_products += [
                    ...
                    if re.sub("id:", "", product_id)
                    not in [str(product["id"]) for product in response_json]
                ]
        else:
    ...
    if missing_products:
        missing_products = [
            re.sub("id:", "", product_id) for product_id in missing_products
        ]
