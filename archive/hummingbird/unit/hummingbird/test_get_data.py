...
from src.hummingbird.get_data import (
    HummingbirdException,
    all_product_data,
    all_product_ids,
)
...
def mocked_html_data(filename):
    with open(
        os.path.join(LOCAL_TEST_DATA_PATH, filename), encoding="utf8"
    ) as htmlfile:
...
def mocked_requests_post(*args, **kwargs):
    ...
    if request_url.startswith(HUMMINGBIRD_WHOLESALE_BASE_URL + "/search"):
        with open(
            os.path.join(LOCAL_TEST_DATA_PATH, "test_data.json"), encoding="utf8"
        ) as jsonfile:
...
class TestAllProductIds(unittest.TestCase):
    ...
    @mock.patch(
        "src.hummingbird.get_data.requests.get", side_effect=mocked_requests_get
    )
    def test_get_all_product_ids(self, mock_get):
        ...
        self.assertIn(
            mock.call(HUMMINGBIRD_WHOLESALE_BASE_URL + HUMMINGBIRD_ALL_PRODUCTS_URL),
            mock_get.call_args_list,
        )
        self.assertIn(
            mock.call(HUMMINGBIRD_WHOLESALE_BASE_URL + "/collections/all?page=4"),
            mock_get.call_args_list,
        )
...
class TestAllProductData(unittest.TestCase):
    ...
    @mock.patch(
        "src.hummingbird.get_data.requests.post", side_effect=mocked_requests_post
    )
    # pylint: disable-next=unused-argument
